...
### `typedef`를 사용한 타입의 추상화 및 코드 가독성 향상 이해

---

## Basics of Structures

- 관련 있는 여러 변수를 **하나의 새 타입으로 묶은 것**
...
- 구조체에서 허용된 연산은 같은 타입 구조체 간 대입 또는 복사, 주소 얻기, 멤버 접근
  - **그 외 연산은 허용하지 않음**
...
- `struct point` 타입을 조작할 수 있는 함수를 소개하면서 구조체를 사용하는 함수의 유형 소개
...
- 구조체 타입 포인터가 가리키는 대상의 멤버는 `->` (arrow) 연산자를 사용해 접근 가능
...
- 평행 구조를 이루는 데이터는 구조체 배열을 사용해 **논리적으로 결합된 하나의 단위**로써 데이터를 조작할 수 있음
...
- C 예약어 입력 빈도 계산 프로그램은 **입력될 데이터가 이미 알려진 경우**였음
...
- 구조체 선언은 해당 타입이 사용될 것임을 미리 알리기 위한 용도
...
- 메모리 관리 함수는 `void *`타입을 반환하므로 형에 독립적이며, **범용적으로 사용 가능**
...
- 해시 탐색 알고리즘의 특징(데이터의 해시 값으로 인덱스를 빠르게 계산할 수 있음)을 사용해 빠르게 탐색 가능
...
- `typedef` 사용 시 **가독성**을 높이며, 이식 가능성 있는 프로그램에서 **기계 의존적인 타입**을 효율적으로 관리할 수 있음
...
- 타입 수와 관계 없이 **하나의 공유된 객체**에 데이터를 관리
...
- 공용체 초기화 시 초기화자의 타입은 공용체의 첫 멤버의 타입을 따름
...
- 명시적으로 2의 배수 형태의 상수를 정의해 비트를 직접 조작할 수 있으나, C는 비트 필드라는 기능을 제공함
