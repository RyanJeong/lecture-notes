...
- Variable Names
- 변수명
- 문자와 숫자를 사용할 수 있으며, 첫 번째 글자는 반드시 영문자 혹은 밑줄(_) 사용
- 
- 
- 
- 변수 이름을 밑줄로 시작하지 말 것
- 표준 라이브러리 내에 미리 정의되어 있는 이름들은 대부분 밑줄로 시작하므로, 이들과의 이름 충돌을 예방하기 위함
- 대문자와 소문자는 서로 구분됨
- int num_seat;
-     int _is_modified;
-     
-     /* error: invalid suffix "nd_trial" on integer constant */
-     int 2nd_trial;
- int a, A;
...
- 변수명은 사용 목적에 맞는 적절한 이름을 사용할 것
- 보통 지역변수(local variables) 이름은 짧게, 전역변수(global variables or external variables) 이름은 길게 작성
- extern int current_index;
-     int i;
-     for (i = 0; i < 10; ++i) {}
- extern int i;
-     int current_index;
-     for (current_index = 0; current_index < 10; ++current_index) {}
...
- 
- 
- 정수형에 한정사(qualifier)를 같이 사용하는 경우:
...
- 
- 
- 정수형 크기는 기계(machine) 혹은 운영체제(OS, operating system)에 따라 다를 수 있음
- 정수형 크기 결정 조건:
- 조건 1. short 형과 int 형은 적어도 16 bits 이상의 크기를 가져야 하며, long 형은 적어도 32 bits 이상의 크기를 가져야 함
- 조건 2. short 형은 int 형보다 클 수 없으며, int 형은 long 형보다 클 수 없음
...
- Data Types and Sizes
- 기본적인 자료형 (Cont’d)
- Christian Wressnegger, Fabian Yamaguchi, Alwin Maier, and Konrad Rieck. 2016. Twice the Bits, Twice the Trouble: Vulnerabilities Induced by Migrating to 64-Bit Platforms.In Proceedings of the 2016 ACM SIGSAC Conference on Computer and Communications Security (CCS '16).
...
- 9
- Data Types and Sizes
- 한정사 부호형(signed), 무부호형(unsigned)
- char형 또는 int형에 적용 가능
- 무부호형은 크기만을 표현하며, 음수를 표현할 수 없음
- e.g. char 형을 표현하는 비트 수는 8 비트이므로, unsigned char 형 값은 modulo 28 연산 결과 중 하나(0 ~ 255)
- 부호형은 양수 뿐만 아니라 음수도 표현 가능
- e.g. signed char 형은 −128에서 127까지의 값 중 하나 (in a two's complement machine)
- char형의 기본 설정은 signed char 또는 unsigned char 
- 기계 종속적(machine-dependent)
- 출력 가능한 ASCII 문자들은 0 ~ 127 사이에 정의되어 있음
...
- 10
- Data Types and Sizes
- 사용중인 환경의 정수형 크기를 확인할 수 있는 예제
- <limits.h>에는 사용중인 환경의 자료형 크기를 나타내는 기호상수들이 정의되어 있음
- e.g. INT_MIN, INT_MAX, etc.
- #include <stdio.h>
-     #include <limits.h>
-     int main(void)
-     {
...
- 11
- Data Types and Sizes
- 사용중인 환경의 정수형 크기를 확인할 수 있는 예제 (Cont’d)
- Ubuntu 18.04 OS에서 실행한 결과:
...
- 다음 코드의 실행 결과는?
- 오버플로우(overflow)
- 실행 결과는 2,500,000,000이 아닌 −1,794,967,296
- int 형이 표현할 수 있는 최대 값은 2,147,483,647
- #include <stdio.h>
-     int main(void)
-     {
-       int num;
...
- 부호형 정수일 때:
- 양수 또는 0을 표현할 경우, MSB는 0
- e.g. 정수 5를 1-byte(char)로 표현
...
- 14
- Data Types and Sizes
- MSB(Most Significant Bit) (Cont’d)
- 부호형 정수일 때:
- 음수를 표현할 경우, MSB는 1
- e.g. 정수 -5를 1-byte(char)로 표현
...
- 무부호형 정수일 때:
- 가중치로 사용
- e.g. 정수 251를 1-byte(char)로 표현
- 
- 
- 
- 
- 1-byte로 표현한 무부호형 정수 251은 1-byte로 표현한 부호형 정수 -5와 표현 방법이 동일함
- 형(type)에 따라 동일한 데이터를 다르게 해석 가능
- 오버플로우는 부호형 정수의 MSB가 연산에 의해 변경되면 발생
...
- 16
- Data Types and Sizes
- 양수 표현 시, 부호형 정수와 무부호형 정수는 MSB를 제외하면 표현 방법 동일
- 부호형 정수 표현 시:
- MSB는 정수의 부호를 결정
- 무부호형 정수 표현 시:
- MSB는 가중치로 사용
- 음수 표현 시, 부호형 정수는 세 가지 표현 방법 사용
...
- Data Types and Sizes
- 2의 보수(2’s complement)
- 각 비트를 반전시킨 뒤 1을 더하면 해당 값의부호가 반전됨
- 부호 비트 사용
- 0: 양수
- 1: 음수
- 0을 표현하는 방법이 유일함
- 
- 오늘날 대부분의 기계에서 사용하는 방법
- 하드웨어로 구현하기 용이
- 00002
- 10002
- 00012
...
- overflow
- e.g. 10002에서 1을 빼면?
- overflow(the term underflow normally refers to floating point numbers only!)
- 00002
- 10002
- 00012
- 11112
...
- 21
- Data Types and Sizes
- xkcd: Can't Sleep. (2023, March 20). Retrieved from https://xkcd.com/571/
...
-       res1 = a + (b + c);
-       res2 = (a + b) + c;
-       printf("The expressions a + (b + c) and (a + b) + c are %s.\n",
-           (res1 == res2) ? "the same" : "not the same");
-       return 0;
-     }
- The expressions a + (b + c) and (a + b) + c are not the same.
