...
- `printf` 함수의 형식 지정자는 세 개의 전달인자를 정수형으로 형식화 후 출력
  - 첫 번째 `%d`는 `i`의 값을 정수형으로 형식화 후 출력
  - 두 번째 `%d`는 `power(2, i)`의 반환값을 정수형으로 형식화 후 출력
  - 세 번째 `%d`는 `power(-3, i)`의 반환값을 정수형으로 형식화 후 출력
