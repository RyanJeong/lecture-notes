...
- **정수형 나눗셈에서의 나머지는 버려지므로, `5 / 9`의 값은 `0`으로 평가됨**
...
- 키보드에서 `'A'`를 입력하면, 실제로 텍스트 입력 스트림에 전달되는 값은 `0x41`
- 텍스트 출력 스트림에 `0x41` 값이 존재할 경우, 모니터에 표현되는 결과는 `'A'`
...
- `Ctrl + D`를 누르면 텍스트 입력 스트림으로 `EOF`가 전달되어 프로그램이 종료됨
...
- **대입 연산자 (`=`)는 논리 연산자 (`!=`)보다 우선순위가 낮음**
  ...
  - `c`에는 문자 값이 되입되는 것이 아닌, 논리 연산자의 평가 결과 (참 또는 거짓)이 대입됨
...
- `char` 형 변수가 아닌 `int`형 변수를 사용한 이유는 **`getchar` 함수가 `EOF`를 반환할 수 있기 때문**
  ...
  - `getchar` 함수는 문자 (256 가지) 뿐만 아니라 입력 스트림 상태 (1 가지)도 반환 (총 257가지)
...
| Data Type  | Size (bytes) | Precision (decimal digits) | Minimum Positive Value | Maximum Value |
|------------|------------|-------------------------|-------------------|---------------|
| `float`    | 4          | ~6–7                    | **1.17549 × 10⁻³⁸**  | **3.40282 × 10³⁸**  |
| `double`   | 8          | ~15–16                   | **2.22507 × 10⁻³⁰⁸** | **1.79769 × 10³⁰⁸** |
...
- 배열 원소 선택은 `NAME[INDEX]`
  - 배열 선언 시 `SIZE` 크기로 선언했다면, **사용 가능한 인덱스는 0 ~ (`SIZE` - 1)**
...
- 함수 안에 선언된 변수는 지역 변수
- 두 함수 (`main`, `power`)은 둘 다 지역변수 `i`를 선언해 사용
...
- `printf` 함수의 형식 지정자는 세 개의 전달인자를 정수형으로 출력
- 첫 번째 `%d`는 `i`, 두 번째 `%d`는 `power(2, i)`의 반환값, 세 번째 `%d`는 `power(-3, i)`와 연관되어 출력됨
...
- `char` 형 배열에 문자열을 저장할 때, 반드시 문자열의 끝에는 널 문자 (`NULL`, `'\0'`)를 기록해야 함
...
- `printf` 함수의 형식 지정자 `%s`를 사용하면 문자열을 출력할 수 있음
