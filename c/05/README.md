...
- `ip`는 같은 타입 객체의 포인터를 가질 수 있는 포인터 변수
...
- 배열 선언 시 `[]` 안의 숫자는 배열의 크기이며, 양의 정수 타입 상수만 사용할 수 있음
- 위 선언은 `int` 타입 객체 10개를 메모리에 **연속적**으로 할당하며, 각 객체는 `a[0]`, `a[1]`, ... , `a[9]`라는 이름을 가짐
...
- 포인터 산술을 수행할 때, 모든 연산은 가리키는 대상의 타입(`T`)의 크기(`sizeof(T)`)를 기준으로 조정됨
...
- 포인터 타입 배열을 사용하면 문자열을 논리적으로 정렬할 수 있음
