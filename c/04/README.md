...
- `auto` 키워드는 지역 변수의 기본 저장 클래스이며, 생략 가능
- 전역에서 사용할 수 없으며, 지역 변수는 기본적으로 `auto`이므로 오늘날 거의 사용되지 않음
...
- CPU의 레지스터를 사용하도록 **추천**하며, 반드시 레지스터를 사용하도록 하는 것이 아님
- 주소를 갖는 메모리와는 다르게, 레지스터는 주소를 갖고 있지 않아 주소 연산자를 사용할 수 없음
- 대부분의 컴파일러는 빌드 과정에서 레지스터를 사용할 변수를 판단 및 자동 적용하므로, 오늘날 거의 사용되지 않음
...
static int global = 100; /* not visible to other files (static variable) */
...
static void reset(void) /* not visible to other files (static function) */
{
    global = 0;
}
```
...
[//]: # (INCLUDE: ./c/04/header/include/foo.h)
...
[//]: # (INCLUDE: ./c/04/07_header_guard/grandparent_no_header_guard.h)

[//]: # (INCLUDE: ./c/04/07_header_guard/parent.h)

[//]: # (INCLUDE: ./c/04/07_header_guard/child.c)

---

## Header Files (Cont'd - 5)

- 헤더 가드는 헤더 파일이 전처리 과정에서 중복 포함되는 것을 방지하기 위한 방법
- 전통적으로 전처리문 중 `#ifndef`를 사용해 구현

[//]: # (INCLUDE: ./c/04/07_header_guard/grandparent.h)
...
[//]: # (INCLUDE: ./c/04/08_printd/printd.c)
...
int main(void)
{
    int i, arr[SIZE] = { 16, 4, 12, 9, 13, 15, 1, 8, 11 };
...
- 식별자를 큰 따옴표 사이에 표현하면 **문자열 상수**가 되어 전처리기가 처리하지 못함
- 식별자를 올바르게 사용하지 않으면 전처리기가 처리하지 못함
...
[//]: # (INCLUDE: ./c/04/token-pasting_operator2.c)
...
[//]: # (INCLUDE: ./c/04/token-pasting_operator1.c)
