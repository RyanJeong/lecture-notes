...
- 정수 타입 크기 지정자로, `h` (e.g., `%hd`)는 `short` 타입 정수를 출력하며 `l` (e.g., `%ld`)은 `long` 타입 정수를 출력
...
- `va_list` 타입은 가변 전달인자를 차례로 접근하기 위한 정보를 담는 타입
- `va_start` 매크로는 `va_list` 타입 객체가 **전달된 이름 없는 객체 중 첫 객체를 가리키도록 초기화**
  - `va_list` 타입 객체를 사용하기 전에 반드시 호출되어야 함
- `va_arg` 매크로는 `va_list` 타입 객체가 가리키는 인자를 지정한 타입으로 읽어옴
  - `va_arg` 매크로가 호출될 때마다 `va_list`타입 객체는 자동으로 다음 인자를 가리킴
...
- 프로그램 종료 시 프로그램에서 열린 모든 출력 파일에 대해 `fclose` 함수가 자동 호출됨
  - 운영체제마다 한 프로그램이 동시에 열 수 있는 파일의 수가 제한되어 있음
  - 불필요한 파일은 다른 프로그램을 위해 명시적으로 닫아주어야 함
- `fclose` 함수는 출력 버퍼에 대기 중인 데이터가 있다면 이를 즉시 파일로 내보냄(flush)
  - 데이터를 내보낼 때 파일로 즉시 전달되지 않고 버퍼에 누적되며, 버퍼가 가득 차거나 `fflush` 또는 `fclose` 호출 시 flush 발생
...
- 오류 처리 개선 및 `stderr` 스트림을 적용한 `cat`
  ...
  - `stderr` 스트림은 표준 출력의 재지정(`>` 또는 `|`)에 영향받지 않으므로 일반 출력과 오류 출력을 구분할 수 있음
...
- 메모리 관리 함수로부터 반환된 포인터는 올바른 메모리 정렬(alignment)을 보장하며, 적절한 타입으로의 캐스팅 필요
...
- `free`: 메모리 관리 함수(`malloc`, `calloc`, `realloc` 등)로 확보한 메모리 해제
