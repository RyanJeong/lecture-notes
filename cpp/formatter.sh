...
#
# Description:
#   Format all .cc and .hpp files under */src/ using clang-format
#   with Google style. File permissions are set to 755 after formatting.
#
# Arguments:
...
readonly CLANG_FORMAT_STYLE="file:${HOME}/clang-format/.clang-format"
...
format_files() {
  ...
  while IFS= read -r -d '' file; do
    ...
    chmod 755 "${file}"
  done < <(find "${scan_dir}" \( -name "*.cc" -o -name "*.hpp" \) -print0 | sort -z)
