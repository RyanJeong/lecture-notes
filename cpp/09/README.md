...
### 자료형이 아닌 템플릿 매개변수: 비-타입 매개변수 (Non-Type Template Parameter)

* 템플릿 매개변수에 **컴파일 시점에 결정되는 상수 값을 매개변수화**하는 형태
  * 비-타입 매개변수에 사용 가능한 자료형 ([Template parameters and template arguments](https://en.cppreference.com/w/cpp/language/template_parameters)):
...
* 템플릿 매개변수에 기본 인자 (값 또는 자료형)를 설정할 수 있음
* 템플릿 기본 매개변수는 오른쪽부터 채워져야 함
* 템플릿 기본 인자로 사용되는 모든 값과 자료형은 컴파일 시점에 확정될 수 있어야 함
...
* 특수화는 특정 자료형에 대해 함수 템플릿의 기본 동작을 재정의함
...
* 템플릿 매개변수를 사용하는 클래스
* 같은 논리의 클래스를 여러 자료형을 사용하는 형태로 인스턴스화가 가능함
...
### 상속 (Inheritance): 비-템플릿 기반 클래스를 상속받는 클래스 템플릿
...
### 상속 (Inheritance): 템플릿 기반 클래스를 상속받는 클래스 템플릿
...
* `CharT`: 문자열을 구성하는 문자 형
* `Traits`: 문자열 특성 및 비교 정책
* `Allocator`: 메모리 관리 정책
* C++ 문자열 형은 [`std::basic_string`](https://en.cppreference.com/w/cpp/string/basic_string.html) 템플릿 클래스를 `char` 형으로 특수화한 것
...
* C++ 표준 입출력 형은 [`std::basic_istream`](https://en.cppreference.com/w/cpp/io/basic_istream.html), [`std::basic_ostream`](https://en.cppreference.com/w/cpp/io/basic_ostream.html) 템플릿 클래스를 `char` 형으로 특수화한 것
