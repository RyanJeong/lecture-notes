...
- 클래스 형 객체가 연산자를 사용할 수 있도록 오버로딩하는 것
...
| `->`      | Postfix| Member-selector    | Overloadable **(not recommended)**|
...
- 전위 증가 / 전위 감소 연산자와 **구분하기 위해 불필요한 (dummy) 정수형 매개변수** 사용
  - 실제 연산에 사용되지 않는 매개변수
...
- `1 + fr`에서의 좌측 피연산자는 기본 자료형이므로, **비멤버 함수** 중 이 표현을 처리할 수 있는 함수가 있는지 탐색
...
- 생성자를 사용하면 다른 자료형을 해당 클래스 형으로 변환 가능
...
- 변환 (conversion) 연산자를 클래스 내부에 정의하면 해당 클래스 객체를 다른 자료형으로 변환 가능
