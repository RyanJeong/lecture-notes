...
- 타입(type)을 이용한 인스턴스(instance, `class`로부터 생성된 객체) 생성
- 타입: 추상화된 것, 인스턴스: 타입을 실체화한 것
...
- C++에서 제공하는 **사용자 정의 타입**
  - 일반 타입으로부터 실체화한 객체: 변수
  - **클래스 타입으로부터 실체화한 객체: 인스턴스**
...
- **행위 없이**(passive) 데이터만을 표현하는 사용자 정의 타입 생성 시 구조체 사용
- 행위가 포함되면서 데이터를 같이 표현하는 사용자 정의 타입 생성 시 클래스 사용
...
- 생성자 선언 시 반환 타입을 사용하지 않음(반환값이 없는 메서드)
- 함수 이름: 클래스 이름과 같음
- `const` 한정자 사용 불가(데이터 멤버 생성 과정에서 부수효과 발생)
...
- 초기화 목록에 선언되지 않은 데이터 멤버: 생성자 본문에서 명시적으로 초기화해야 함
  - 초기화하지 않은 기본 타입 멤버는 쓰레기 값을, 클래스 타입 멤버는 생성자 호출을 통해 초기화함
...
- 소멸자 선언 시 반환 타입을 사용하지 않음(반환값이 없는 메서드)
- 함수 이름: 클래스 이름과 같으나, 함수 이름 앞에 `~` 기호 사용
- `const` 한정자를 사용 불가(데이터 멤버 소멸 과정에서 부수효과 발생)
...
- 라이브러리 파일은 **역컴파일이 어려우므로** 알고리즘 보호에 유리함
